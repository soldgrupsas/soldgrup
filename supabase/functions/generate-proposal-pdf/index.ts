...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseKey) {
      throw new Error('Variables de entorno SUPABASE_URL o SUPABASE_SERVICE_ROLE_KEY no están configuradas');
    }
    ...
    if (proposalError) {
      console.error('Error consultando propuesta:', proposalError);
      throw new Error(`Error al consultar propuesta: ${proposalError.message}`);
    }
    ...
    // Fetch related data
    console.log('📦 Consultando datos relacionados...');
    const [{ data: items, error: itemsError }, { data: images, error: imagesError }, { data: equipmentDetails, error: equipmentDetailsError }] = await Promise.all([
    ...
    if (itemsError) {
      console.warn('Advertencia al consultar items:', itemsError);
    }
    if (imagesError) {
      console.warn('Advertencia al consultar imágenes:', imagesError);
    }
    if (equipmentDetailsError) {
      console.warn('Advertencia al consultar equipment_details:', equipmentDetailsError);
    }
    ...
    if (equipmentDetails && equipmentDetails.length > 0) {
      ...
      if (equipmentIds.length > 0) {
        ...
        if (equipmentDataError) {
          console.warn('Advertencia al consultar equipment:', equipmentDataError);
        }
    ...
    console.log(`✅ Datos consultados: ${items?.length || 0} items, ${images?.length || 0} imágenes, ${equipment?.length || 0} equipos`);
